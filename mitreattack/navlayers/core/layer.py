...
class Layer:
    ...
    def _build(self):
        ...
        try:
            ...
            handler(type(self).__name__, "Layer is malformed: {}. " "Unable to load.".format(e))
            self.__layer = None
            return
        except KeyError as e:
            handler(type(self).__name__, "Layer is missing parameters: {}. " "Unable to load.".format(e))
            self.__layer = None
            return
            ...
            if key not in ["name", "domain"]:
                try:
                    ...
                    if self.strict:
                        handler(
                            type(self).__name__,
                            "{} encountered [{}]. " "Unable to load.".format(str(e.__class__.__name__), e),
                        )
                        handler(type(self).__name__, "Full Traceback - {}".format(traceback.format_exc()))
                        self.__layer = None
                        return
